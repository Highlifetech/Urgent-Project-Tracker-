...
def get_todays_meetings():
    ...
    try:
              ...
              # Calculate ET offset (roughly UTC-4 or UTC-5)
...
def filter_important_emails(emails, anthropic_client=None):
    ...
    if not emails or not anthropic_client:
              return emails[:10]  # fallback: return first 10

    # Build a summary of emails for Claude to evaluate
    email_summaries = []
    ...
    try:
              response = anthropic_client.messages.create(
                            ...
                            messages=[{
                                              "role": "user",
                                              "content": f"""You are an executive assistant for a promotional products company (Highlifetech/HLT).
                                              Review these emails and identify which ones are important or project-related.
                                              Important = client emails, order updates, production issues, shipping, quotes, invoices, vendor communications, urgent requests.
                                              ...
                                              [{{"num": 1, "reason": "Client asking about order status"}}, {{"num": 3, "reason": "Vendor quote for new project"}}]
